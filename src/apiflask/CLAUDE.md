...
Scope: the APIFlask framework source. For repo-wide commands see the root `CLAUDE.md`.
...
- Add or update tests under `tests/` (file-per-feature, see `tests/CLAUDE.md`).
- Public functions/classes need docstrings — they are picked up by `mkdocstrings` and rendered into the docs site under `docs/api/`. When adding a new public module, add a matching stub under `docs/api/` (see `docs/CLAUDE.md`).
