...
- Add support for API key auth with `APIKeyHeaderAuth`, `APIKeyCookieAuth`, and `APIKeyQueryAuth`. The API key auth by HTTPTokenAuth is deprecated. Support for a runtime choice of Authentication methods with `MultiAuth` ([issue #604][issue_604]).
