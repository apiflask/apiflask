.. APIFlask documentation master file, created by
...
The main function of the ``APIFlask`` instance is to gather all the information
...
In addition to ducmentation, ``APIFlask`` allows the application to
...
- The project's name and version are obtained from the ``APIFLASK_TITLE`` and
  ``APIFLASK_VERSION`` configuration items respectively.
...
- If a route belongs to a blueprint, the corresponding path is tagged with the
  ...
  ``APIFLASK_TAGS`` configuration item can be used to provide a custom ordering
  for tags.
